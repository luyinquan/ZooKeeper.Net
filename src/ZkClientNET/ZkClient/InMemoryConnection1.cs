...
{
    public class InMemoryConnection1 : IZkConnection
    {
        ...
        private class EventThread
        {
            ...
            public EventThread(IWatcher watcher)
            {
                _watcher = watcher;
        ...
        public void Connect(IWatcher watcher)
        {
            try
            {
                lock (_lock)
                {
                    ...
                    _eventThread.Start();
                    _eventThread.Send(new WatchedEvent(KeeperState.SyncConnected, EventType.None, null));
                }
            }
            finally
            {
        ...
        private void CheckWatch(HashSet<string> watches, string path, EventType eventType)
        {
            ...
            {
                watches.Remove(path);
                _eventThread.Send(new WatchedEvent(KeeperState.SyncConnected, eventType, path));
            }
        }
        ...
        {
            DataAndVersion node = _data[path];
            if (node == null)
            {
                return;
            }
