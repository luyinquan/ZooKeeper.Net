...
namespace ZkClientNET.ZkClient
{
    ...
    public class ZkClient : IWatcher
    {
        ...
        /// <param name="persistent"></param>
        /// <returns></returns>
        private string Create(string path, object data, List<ACL> acl, CreateMode mode)
        {
            if (path == null)
            {
                throw new ArgumentNullException("Missing value for path");
        ...
        {
            ConcurrentHashSet<IZkDataListener> dataListeners = _dataListener[path];
            if (dataListeners != null && dataListeners.Count > 0)
            {
                return true;
            }
            ConcurrentHashSet<IZkChildListener> childListeners = _childListener[path];
        ...
        private bool Delete(string path, int version)
        {
            try
            {
                RetryUntilConnected(() =>
